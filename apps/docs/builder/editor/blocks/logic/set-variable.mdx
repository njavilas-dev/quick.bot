...
This preset value will save the entire conversation transcript in a variable. It is super useful to
provide context to an AI block or to send it as a recap with the
...
This will set your variable with the current result ID. The result ID is the ID that corresponds to
a row of your [Results](../../../results/table) table. It can be considered like a User ID for
the currently chatting user.

## Yesterday, Now, Tomorrow

This will set your variable with the specified date and time in ISO format. You can optionally
provide a time zone to convert the date to the specified time zone.
...
It will set your variable with either one of these values based on the user's time of the day:
...
By default, new variables are not persisted in the [Results](../../../results/table) table. They are only
stored for the current user chat session. Enabling this option will save the variable in the
`Results` table.
