...
import { z } from 'zod'
import { continueChat as continueChatFn } from '@quickbot.io/bot-engine/apiHandlers/continueChat'
...
export const continueChat = publicProcedure
  ...
  .input(
    z.object({
      ...
      sessionId: z
        .string()
        .describe('The session ID you got from the [startChat](./start-chat) response.'),
      textBubbleContentFormat: z.enum(['richText', 'markdown']).default('richText'),
    }),
  )
  .output(continueChatResponseSchema)
  .mutation(
    async ({ input: { sessionId, message, textBubbleContentFormat }, ctx: { origin, res } }) => {
