...
import { badRequest, forbidden, initMiddleware, methodNotAllowed } from '@quickbot.io/lib/api'
import { decrypt } from '@quickbot.io/lib/api/encryption/decrypt'
import Stripe from 'stripe'

import Cors from 'cors'
import { PaymentInputBlock, StripeCredentials, Variable } from '@quickbot.io/schemas'
...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  if (req.method === 'POST') {
    ...
    const currency = inputOptions.currency ?? defaultPaymentInputOptions.currency
    ...
    const receiptEmail = parseVariables(variables)(inputOptions.additionalInformation?.email)
    try {
