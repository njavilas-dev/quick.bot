...
interface Actions<T extends { updatedAt: Date }> {
...
interface History<T extends { updatedAt: Date }> {
  past: T[]
  present: T | undefined
  future: T[]
}
...
export const useUndo = <T extends { updatedAt: Date }>(
  ...
  const set = useCallback(
    (newPresentArg: T | ((current: T) => T) | undefined) => {
      ...
      const newPresent =
        typeof newPresentArg === 'function' ? newPresentArg(presentRef.current as T) : newPresentArg

      if (
        newPresent &&
        present &&
        dequal(JSON.parse(JSON.stringify(newPresent)), JSON.parse(JSON.stringify(present)))
      ) {
        return
      }
  ...
  return [history.present, { set, undo, redo, flush, setUpdateDate, canUndo, canRedo }]
}
