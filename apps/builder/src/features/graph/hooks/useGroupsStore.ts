...
import { Edge, Group, GroupV6, Variable } from '@quickbot.io/schemas'
...
type Store = {
  ...
  // TO-DO: remove once QuickBot provider is migrated to a Zustand store. We will be able to get it internally in the store (if mutualized).
        ...
        groupsCoordinates: groupsCoordinates
          ? {
              ...
              ...focusedGroups.reduce(
                (coords, groupId) => {
                  const currentCoords = groupsCoordinates[groupId]
                  if (!currentCoords) return coords
                  return {
                    ...coords,
                    [groupId]: {
                      x: Number((currentCoords.x + delta.x).toFixed(2)),
                      y: Number((currentCoords.y + delta.y).toFixed(2)),
                    },
                  }
                },
                groupsCoordinates,
