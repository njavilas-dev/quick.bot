...
export const BlockNodesList = ({ blocks, groupIndex, groupRef }: Props) => {
  const { draggedBlock, setDraggedBlock, draggedBlockType, mouseOverGroup, setDraggedBlockType } =
    useBlockDnd()
  const { bot, createBlock, detachBlockFromGroup } = useBot()
  ...
  const handleMouseMoveOnGroup = (event: MouseEvent) => {
    ...
    setExpandedPlaceholderIndex(computeNearestPlaceholderIndex(event.pageY, placeholderRefs))
  }
  ...
  const handleMouseUpOnGroup = (e: MouseEvent) => {
    ...
    const blockId = createBlock((draggedBlock || draggedBlockType) as BlockV6 | BlockV6['type'], {
      groupIndex,
      blockIndex,
    })
  ...
  const handleBlockMouseDown =
    (blockIndex: number) =>
    ({ relative, absolute }: { absolute: Coordinates; relative: Coordinates }, block: BlockV6) => {
  ...
  useEventListener(
    mouseOverGroup ? mouseOverGroup.element : null,
    'mouseup',
    mouseOverGroup ? handleMouseUpOnGroup : () => {},
    {
      capture: true,
    },
  )
