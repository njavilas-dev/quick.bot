...
import { BlockSource, Edge as EdgeProps, GroupV6 } from '@quickbot.io/schemas'
...
export const Edges = ({ edges, groups }: Props) => {
  const { connectingIds, selectedEdge } = useGraph()
  ...
  return (
    ...
    >
      ...
      {(() => {
        const selectedEdgeObj = edges.find((edge) => edge.id === selectedEdgeId)
        return selectedEdgeObj && renderEdge(selectedEdgeObj)
      })()}
      <marker
        id="arrow"
        refX="8"
