...
export const DrawingEdge = ({ connectingIds }: Props) => {
  ...
  const { sourceEndpointYOffsets: sourceEndpoints, targetEndpointYOffsets: targetEndpoints } =
    useEndpoints()
  ...
  const path = useMemo(() => {
    if (!sourceTop || !sourceElementCoordinates || !mousePosition || !connectingIds?.source)
      return ``
    ...
    return targetGroupCoordinates
      ? computeConnectingEdgePath({
        ...
        elementWidth: 'eventId' in connectingIds.source ? eventWidth : groupWidth,
        sourceTop,
        targetTop,
