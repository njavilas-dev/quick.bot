import { groupAnchorsOffset, stubLength, groupWidth } from '../constants'
...
export const getAnchorsPosition = ({
  ...
  // Determine sourceType based on relative positions
  // Normalize source position as if it had the same width as target for symmetrical comparison
  const normalizedSourceX = sourceGroupCoordinates.x + (elementWidth - groupWidth) / 2

  let sourceType: 'right' | 'left' = 'right'
  if (normalizedSourceX > targetGroupCoordinates.x) {
...
const computeGroupTargetPosition = ({
  ...
  // Normalize source position as if it had the same width as target for symmetrical comparison
  const normalizedSourceX = sourceGroupPosition.x + (elementWidth - groupWidth) / 2

  const isTargetGroupBelow =
    targetGroupPosition.y > sourceOffsetY &&
    targetGroupPosition.x < normalizedSourceX + groupWidth + stubLength &&
    targetGroupPosition.x > normalizedSourceX - stubLength
  const isTargetGroupToTheRight = targetGroupPosition.x < normalizedSourceX
  const isTargettingGroup = !targetOffsetY

  if (isTargetGroupBelow && isTargettingGroup) {
    const isExterior =
      targetGroupPosition.x < normalizedSourceX - groupWidth / 2 - stubLength ||
      targetGroupPosition.x > normalizedSourceX + groupWidth / 2 + stubLength
    const targetPosition = parseGroupAnchorPosition(targetGroupPosition, 'top')
    return { totalSegments: isExterior ? 2 : 4, targetPosition }
  } else {
    const isExterior =
      targetGroupPosition.x < normalizedSourceX - groupWidth ||
      targetGroupPosition.x > normalizedSourceX + groupWidth
