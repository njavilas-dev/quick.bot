import { AbTestBlock, BlockV6, BlockWithItems, ItemV6 } from '@quickbot.io/schemas'
import {
  createContext,
  Dispatch,
  ReactNode,
...
export type BlockWithCreatableItems = Exclude<BlockWithItems, { type: LogicBlockType.AB_TEST }>
...
const graphDragAndDropContext = createContext<{
  ...
  setDraggedBlock: Dispatch<SetStateAction<(BlockV6 & { groupId: string }) | undefined>>
...
export const GraphDragAndDropProvider = ({ children }: { children: ReactNode }) => {
  ...
  return (
    <graphDragAndDropContext.Provider
      value={{
        draggedBlock,
        setDraggedBlock,
        draggedBlockType,
...
export const useDragDistance = ({
  ...
  useEffect(() => {
    ...
    const triggerDragCallbackIfMouseMovedEnough = (e: MouseEvent) => {
      ...
      if (
        Math.abs(mouseDownPosition.current.absolute.x - clientX) > distanceTolerance ||
        Math.abs(mouseDownPosition.current.absolute.y - clientY) > distanceTolerance
      ) {
        triggered = true
    ...
    return () => {
      document.removeEventListener('mousemove', triggerDragCallbackIfMouseMovedEnough)
    }
...
export const useBlockDnd = () => useContext(graphDragAndDropContext)
