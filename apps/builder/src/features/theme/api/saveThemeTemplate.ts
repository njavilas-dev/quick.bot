...
import { ThemeTemplate, themeTemplateSchema } from '@quickbot.io/schemas'
import { z } from 'zod'
...
export const saveThemeTemplate = authenticatedProcedure
  ...
  .mutation(async ({ input: { themeTemplateId, workspaceId, ...data }, ctx: { user } }) => {
    ...
    const themeTemplate = (await prisma.botThemeTemplate.upsert({
