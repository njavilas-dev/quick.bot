import { InputText, InputTextCopy } from '@urbiport/ui'
...
export const AccountApiTokensModal = ({ userId, isOpen, onClose, onNewToken }: Props) => {
        ...
        {newTokenValue ? (
          <ModalBody as={Stack} spacing="4">
            <Text>
              ...
              <strong>{t('account.apiTokens.createModal.securityWarning')}</strong>
            </Text>
            <InputTextCopy readOnly defaultValue={newTokenValue} />
          </ModalBody>
        ) : (
          <ModalBody as="form" onSubmit={createToken}>
            <Text mb="4">{t('account.apiTokens.createModal.nameInput.label')}</Text>
            <InputText
              ref={inputRef}
              placeholder={t('account.apiTokens.createModal.nameInput.placeholder')}
              onChange={setName}
            />
          </ModalBody>
        )}
