import { MenuList, MenuItem, useDisclosure, Menu, MenuButton, HStack, Text } from '@chakra-ui/react'
import { RepeatIcon, ChevronRightIcon } from '@urbiport/icons'
...
export const ForgedBlockTurnIntoMenu = ({ block, onTurnIntoClick }: Props) => {
  ...
  if (!actionDef || !actionDef?.turnableInto || actionDef?.turnableInto.length === 0) return null
  return (
    <Menu isOpen={isOpen} placement="right" onClose={onClose}>
      <MenuButton
        as={MenuItem}
        onClick={onOpen}
      ...
      <MenuList onMouseEnter={handleMouseEnter} onMouseLeave={debounceSubMenuClose}>
...
const TurnIntoMenuItem = ({
  ...
  return (
    <MenuItem icon={<ForgedBlockIcon type={blockType} />} onClick={() => onClick(blockSchema)}>
      {blockDef.name}
    </MenuItem>
  )
}
