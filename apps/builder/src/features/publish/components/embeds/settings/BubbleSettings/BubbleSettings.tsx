import { Stack, HStack, Flex, Text, Image, chakra } from '@chakra-ui/react'
import { BubbleProps } from '@urbiport/nextjs'
import { isDefined, isSvgSrc } from '@quickbot.io/lib'
...
type Props = {
  ...
  onPreviewMessageChange: (previewMessage: BubbleProps['previewMessage']) => void
}
        ...
        <Stack alignItems="flex-end">
          {isDefined(previewMessage) && (
            ...
            >
              ...
              <Text color={theme?.previewMessage?.textColor}>{previewMessage.message}</Text>
            </HStack>
          )}
          <Flex
            align="center"
  ...
  if (!buttonTheme?.customIconSrc)
    return (
      <svg
        viewBox="0 0 24 24"
        style={{
          stroke: buttonTheme?.backgroundColor ?? '#fff',
          transition: 'all 0.2s ease-in-out',
        }}
  ...
  if (
    ...
    return (
      <Image
        ...
        borderRadius={isSvgSrc(buttonTheme.customIconSrc) ? undefined : 'full'}
