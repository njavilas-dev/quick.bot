...
const parseButtonTheme = (button: NonNullable<BubbleProps['theme']>['button']): string => {
  if (!button) return ''
  const { backgroundColor, iconColor, customIconSrc } = button
  const backgroundColorLine = parseStringParam('backgroundColor', backgroundColor)
...
const parsePreviewMessageTheme = (
  ...
  const { backgroundColor, closeButtonBackgroundColor, closeButtonIconColor, textColor } =
    previewMessage
  const backgroundColorLine = parseStringParam('backgroundColor', backgroundColor)
  ...
  const closeButtonIconColorLine = parseStringParam('closeButtonIconColor', closeButtonIconColor)
...
const parseChatWindowTheme = (chatWindow: NonNullable<BubbleProps['theme']>['chatWindow']) => {
  if (!chatWindow) return ''
  const backgroundColorLine = parseStringParam('backgroundColor', chatWindow.backgroundColor)
...
const parsePreviewMessage = (previewMessage: BubbleProps['previewMessage']): string => {
  if (!previewMessage) return ''
...
export const parseInitBubbleCode = ({ bot, apiHost, previewMessage, theme }: BubbleProps) => {
  const botProps = parseBotProps({ bot: bot, apiHost })
  const bubbleProps = parseBubbleProps({ previewMessage, theme })

  return prettier.format(`QuickBot.initBubble({${botProps}${bubbleProps}});`, {
    parser: 'babel',
    plugins: [parserBabel],
  })
}
...
const parseReactPreviewMessage = (previewMessage: BubbleProps['previewMessage']): string => {
  if (!previewMessage) return ''
  const { message, autoShowDelay, avatarUrl } = previewMessage
  const messageLine = parseStringParam('message', message)
  const autoShowDelayLine = parseNumberOrBoolParam('autoShowDelay', autoShowDelay)
...
export const parseReactBubbleProps = ({ bot, apiHost, previewMessage, theme }: BubbleProps) => {
