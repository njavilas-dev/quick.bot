...
import { GithubIcon } from '@urbiport/icons'
...
import { KeycloackLogo, GitlabLogo, FacebookLogo, AzureAdLogo, GoogleLogo } from '@urbiport/icons'
...
export const SocialLoginButtons = ({ providers }: Props) => {
  ...
  const handleSignIn = async (provider: string) => {
    ...
    await signIn(provider, {
      callbackUrl:
        query.callbackUrl?.toString() ?? `/bots?${stringify(omit(query, 'error', 'callbackUrl'))}`,
    })
  ...
  return (
    <Stack>
      <DividerWithText mt="6" mb={6}>
        {t('auth.orEmailLabel')}
      </DividerWithText>
      {providers?.github && (
        <Button
          leftIcon={<GithubIcon color="black" />}
      ...
      {providers?.google && (
        <Button
          ...
          data-testid="google"
          isLoading={['loading', 'authenticated'].includes(status) || authLoading === 'google'}
          variant="outline"
        >
      ...
      {providers?.facebook && (
        <Button
          ...
          isLoading={['loading', 'authenticated'].includes(status) || authLoading === 'facebook'}
          variant="outline"
        >
      ...
      {providers?.gitlab && (
        <Button
          ...
          data-testid="gitlab"
          isLoading={['loading', 'authenticated'].includes(status) || authLoading === 'gitlab'}
          variant="outline"
        >
      ...
      {providers?.['azure-ad'] && (
        <Button
          ...
          isLoading={['loading', 'authenticated'].includes(status) || authLoading === 'azure-ad'}
          variant="outline"
        >
      ...
      {providers?.['custom-oauth'] && (
        <Button
          ...
          isLoading={
            ['loading', 'authenticated'].includes(status) || authLoading === 'custom-oauth'
          }
          variant="outline"
        >
      ...
      {providers?.keycloak && (
        <Button
          ...
          isLoading={['loading', 'authenticated'].includes(status) || authLoading === 'keycloak'}
          variant="outline"
        >
