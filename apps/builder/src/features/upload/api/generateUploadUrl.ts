...
export type FilePathUploadProps = z.infer<typeof inputSchema.shape.filePathProps>
...
const parseFilePath = async ({
  ...
  if (!('botId' in input)) {
    ...
    if (!workspace || isWriteWorkspaceForbidden(workspace, { id: authenticatedUserId }))
      throw new TRPCError({
        code: 'NOT_FOUND',
  ...
  if (!('blockId' in input)) {
    return `public/workspaces/${input.workspaceId}/bots/${input.botId}/${input.fileName}`
  }
  return `public/workspaces/${input.workspaceId}/bots/${input.botId}/blocks/${input.blockId}${
