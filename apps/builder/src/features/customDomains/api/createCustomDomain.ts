...
import { z } from 'zod'
import { customDomainSchema } from '@quickbot.io/schemas/features/customDomains'
...
export const createCustomDomain = authenticatedProcedure
  ...
  .mutation(async ({ input: { workspaceId, name }, ctx: { user } }) => {
    ...
    const existingCustomDomain = await prisma.workspaceCustomDomain.findFirst({
      where: { name },
    })
    ...
    const customDomain = await prisma.workspaceCustomDomain.create({
      data: {
        name,
        workspaceId,
      },
