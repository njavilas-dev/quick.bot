...
} from '@quickbot.io/schemas/features/customDomains'
...
export const verifyCustomDomain = authenticatedProcedure
  ...
  .query(async ({ input: { workspaceId, name }, ctx: { user } }) => {
    ...
    const normalizedDomainJson = {
      name: domainJson.name || '',
      apexName: domainJson.apexName || '',
      projectId: domainJson.projectId || '',
      redirect: domainJson.redirect || '',
      redirectStatusCode: domainJson.redirectStatusCode || 0,
      gitBranch: domainJson.gitBranch || '',
      updatedAt: domainJson.updatedAt || Date.now(),
      createdAt: domainJson.createdAt || Date.now(),
      verified: domainJson.verified || false,
      error: domainJson.error || null,
    }

    return {
      status,
      domainJson: normalizedDomainJson,
    }
  })
