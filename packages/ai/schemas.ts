...
const parameterBase = {
  ...
  required: option.boolean.layout({
    direction: 'row',
    label: 'Is required?',
    moreInfoTooltip: 'If set to false, there is a chance the variable will be empty',
  }),
}

const toolParametersSchema = option
  .array(
    option.discriminatedUnion('type', [
      ...
      option
        .object({
          ...
          values: option.array(option.string).layout({ itemLabel: 'possible value' }),
        })
        .extend(parameterBase),
    ]),
  )
