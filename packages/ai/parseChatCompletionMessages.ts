...
const parseDialogueMessage = async ({
  ...
  return Promise.all(
    dialogueArr.map<Promise<CoreUserMessage | CoreAssistantMessage | undefined>>(
      async (dialogueItem, index) => {
        if (!dialogueItem) return
        const startsBy = message.startsBy ?? 'user'
        if (index === 0 && startsBy === 'assistant')
          return { role: 'assistant' as const, content: dialogueItem }
        if (index % (startsBy === 'assistant' ? 1 : 2) === 0) {
          return {
            role: 'user' as const,
