...
export const isRefObject: <T>(ref: React.Ref<T>) => ref is React.RefObject<T> = <T>(
  ref: React.Ref<T>
...
export const isVariableString = (str?: string | number): boolean => {
  return str ? /^{{.+}}$/g.test(str.toString()) : false
}
...
export const isPublicIdValid = (str: string) =>
