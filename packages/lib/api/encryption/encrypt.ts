...
export const encrypt = async (data: object): Promise<{ encryptedData: string; iv: string }> => {
  ...
  const encryptedBuffer = await crypto.subtle.encrypt({ name: algorithm, iv }, key, encodedData)
