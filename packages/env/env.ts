...
const baseEnv = {
  server: {
    TEST_ENV: boolean.optional().default(process.env.NODE_ENV === 'development' ? 'true' : 'false'),
    ...
    DEFAULT_WORKSPACE_PLAN: z
      .enum(['FREE', 'PERSONAL', 'BUSINESS', 'ENTERPRISE', 'UNLIMITED'])
      .refine((str) => ['FREE', 'PERSONAL', 'BUSINESS', 'ENTERPRISE', 'UNLIMITED'].includes(str))
      .default('FREE'),
  ...
  client: {
    ...
    NEXT_PUBLIC_SUPPORT_BOT: z.string().min(1).optional(),
    NEXT_PUBLIC_BETA_ENV: boolean.optional().default('true'),
  },
  runtimeEnv: {
    NEXT_PUBLIC_VIEWER_URL: getRuntimeVariable('NEXT_PUBLIC_VIEWER_URL'),
    NEXT_PUBLIC_ONBOARDING_BOT_ID: getRuntimeVariable('NEXT_PUBLIC_ONBOARDING_BOT_ID'),
    ...
    NEXT_PUBLIC_VIEWER_404_TITLE: getRuntimeVariable('NEXT_PUBLIC_VIEWER_404_TITLE'),
    NEXT_PUBLIC_VIEWER_404_SUBTITLE: getRuntimeVariable('NEXT_PUBLIC_VIEWER_404_SUBTITLE'),
    NEXT_PUBLIC_SUPPORT_BOT: getRuntimeVariable('NEXT_PUBLIC_SUPPORT_BOT'),
    NEXT_PUBLIC_BETA_ENV: getRuntimeVariable('NEXT_PUBLIC_BETA_ENV'),
  },
}
const githubEnv = {
  server: {
...
const googleEnv = {
  server: {
    ...
    PLAYWRIGHT_GOOGLE_ACCESS_TOKEN: z.string().min(1).optional(),
    PLAYWRIGHT_GOOGLE_REFRESH_TOKEN: z.string().min(1).optional(),
  },
  client: {
    NEXT_PUBLIC_GOOGLE_API_KEY: z.string().min(1).optional(),
  },
  runtimeEnv: {
    NEXT_PUBLIC_GOOGLE_API_KEY: getRuntimeVariable('NEXT_PUBLIC_GOOGLE_API_KEY'),
  },
}

const playwrightEnv = {
  server: {
    PLAYWRIGHT_STRIPE_PERSONAL_CUSTOMER_ID: z.string().min(1).optional(),
    PLAYWRIGHT_STRIPE_BUSINESS_CUSTOMER_ID: z.string().min(1).optional(),
    PLAYWRIGHT_STRIPE_ENTERPRISE_CUSTOMER_ID: z.string().min(1).optional(),
  },
  client: {},
  runtimeEnv: {},
}
...
const stripeEnv = {
  server: {
    ...
    STRIPE_PERSONAL_PRICE_ID: z.string().min(1).optional(),
    STRIPE_PERSONAL_CHATS_PRICE_ID: z.string().min(1).optional(),
    STRIPE_BUSINESS_PRICE_ID: z.string().min(1).optional(),
    STRIPE_BUSINESS_CHATS_PRICE_ID: z.string().min(1).optional(),
    STRIPE_ENTERPRISE_PRICE_ID: z.string().min(1).optional(),
    STRIPE_ENTERPRISE_CHATS_PRICE_ID: z.string().min(1).optional(),
    STRIPE_BILLING_ADDRESS_COLLECTION: z.enum(['auto', 'required']).default('auto'),
    STRIPE_AUTOMATIC_TAXES: boolean.optional().default('false'),
  },
  client: {
...
const unsplashEnv = {
  ...
  runtimeEnv: {
    NEXT_PUBLIC_UNSPLASH_APP_NAME: getRuntimeVariable('NEXT_PUBLIC_UNSPLASH_APP_NAME'),
    NEXT_PUBLIC_UNSPLASH_ACCESS_KEY: getRuntimeVariable('NEXT_PUBLIC_UNSPLASH_ACCESS_KEY'),
  },
}
...
const whatsAppEnv = {
  server: {
    ...
    WHATSAPP_PREVIEW_TEMPLATE_LANG: z.string().min(1).optional().default('en_US'),
    WHATSAPP_CLOUD_API_URL: z.string().url().optional().default('https://graph.facebook.com'),
...
const posthogEnv = {
  client: {
    ...
    NEXT_PUBLIC_POSTHOG_HOST: z.string().min(1).optional().default('https://us.i.posthog.com'),
  },
  runtimeEnv: {
...
const tolgeeEnv = {
  client: {
    ...
    NEXT_PUBLIC_TOLGEE_API_URL: z
      ...
      .url()
      .optional()
      .default('https://tolgee.server.urbiport.com'),
  },
  runtimeEnv: {
    NEXT_PUBLIC_TOLGEE_API_KEY: getRuntimeVariable('NEXT_PUBLIC_TOLGEE_API_KEY'),
    NEXT_PUBLIC_TOLGEE_API_URL: getRuntimeVariable(
      'NEXT_PUBLIC_TOLGEE_API_URL',
      'https://tolgee.server.urbiport.com',
    ),
  },
}
...
const emailServiceEnv = {
  server: {
    EMAIL_HOST: z.string().optional(),
    EMAIL_PORT: z.string().optional(),
    EMAIL_SECURE: boolean.optional().default('false'),
...
const authCredentialEnv = {
  server: {
    CREDENTIALS_AUTH: boolean.optional().default('false'),
  },
}
...
export const env = createEnv({
  server: {
    ...
    ...telemetryEnv.server,
    ...keycloakEnv.server,
    ...emailServiceEnv.server,
    ...authCredentialEnv.server,
  },
  client: {
    ...baseEnv.client,
    ...smtpEnv.client,
    ...googleEnv.client,
    ...playwrightEnv.client,
    ...stripeEnv.client,
    ...giphyEnv.client,
    ...vercelEnv.client,
    ...unsplashEnv.client,
