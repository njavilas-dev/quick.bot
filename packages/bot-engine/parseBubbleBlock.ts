...
import { BubbleBlock, Variable, ContinueChatResponse, Bot } from '@quickbot.io/schemas'
import { deepParseVariables } from '@quickbot.io/variables/deepParseVariables'
import { isDefined, isEmpty, isNotEmpty } from '@quickbot.io/lib/utils'
import {
...
import { convertMarkdownToRichText } from '@quickbot.io/lib/markdown/convertMarkdownToRichText'
import { convertRichTextToMarkdown } from '@quickbot.io/lib/markdown/convertRichTextToMarkdown'
import { isSingleVariable } from '@quickbot.io/variables/isSingleVariable'
...
export const parseBubbleBlock = (
  ...
  { version, variables, botVersion, textBubbleContentFormat }: Params,
  ...
  switch (block.type) {
    case BubbleBlockType.TEXT: {
      if (version === 1)
        return {
          ...
          content: {
            type: 'richText',
            richText: (block.content?.richText ?? []).map(deepParseVariables(variables)),
          },
        }

      const richText = parseVariablesInRichText(block.content?.richText ?? [], {
        variables,
        takeLatestIfList: botVersion !== '6',
      }).parsedElements
      return {
        ...block,
        content:
    ...
    case BubbleBlockType.VIDEO: {
      const parsedContent = block.content ? deepParseVariables(variables)(block.content) : undefined
...
export const parseVariablesInRichText = (
  ...
  for (const element of elements) {
    if ('text' in element) {
      ...
      parsedVariableIds.push(...variablesInText.map((v) => v.variableId).filter(isDefined))
      ...
      for (const variableInText of variablesInText) {
        index += 1
        const textBeforeVariable = text.substring(lastTextEndIndex, variableInText.startIndex)
        const textAfterVariable =
          index === variablesInText.length - 1 ? text.substring(variableInText.endIndex) : undefined
        lastTextEndIndex = variableInText.endIndex
        const isStandaloneElement =
          isEmpty(textBeforeVariable) && isEmpty(textAfterVariable) && variablesInText.length === 1
        const variableElements = convertMarkdownToRichText(
          isStandaloneElement ? variableInText.value : variableInText.value.replace(/[\n]+/g, ' '),
        )

        const variableElementsWithStyling = applyElementStyleToDescendants(variableElements, {
          bold: element.bold,
          italic: element.italic,
          underline: element.underline,
        })

        if (isStandaloneElement && !element.bold && !element.italic && !element.underline) {
    ...
    const { parsedElements: parsedChildren, parsedVariableIds: parsedChildrenVariableIds } =
    ...
    parsedElements.push({
      ...element,
      url: element.url ? parseVariables(variables)(element.url as string) : undefined,
      type,
...
const applyElementStyleToDescendants = (
  ...
  variableElements.map((variableElement) => {
    ...
    return {
      ...variableElement,
      children: applyElementStyleToDescendants(variableElement.children, styles),
    }
  })
