...
export const continueChat = async ({
  ...
  if (session?.state.allowedOrigins && session.state.allowedOrigins.length > 0) {
    if (origin && session.state.allowedOrigins.includes(origin)) corsOrigin = origin
  ...
  const isPreview = isNotDefined(session.state.botsQueue[0].resultId)
