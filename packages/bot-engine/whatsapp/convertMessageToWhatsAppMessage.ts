import { ContinueChatResponse, EmbeddableVideoBubbleContentType } from '@quickbot.io/schemas'
import { WhatsAppSendingMessage } from '@quickbot.io/schemas/features/whatsapp'
import { isSvgSrc } from '@quickbot.io/lib/utils'
...
export const convertMessageToWhatsAppMessage = (
  ...
  switch (message.type) {
    case BubbleBlockType.TEXT: {
      if (message.content.type === 'markdown') throw new Error('Expect rich text message')
      if (!message.content.richText || message.content.richText.length === 0) return null
      return {
        type: 'text',
        text: {
    ...
    case BubbleBlockType.IMAGE: {
      if (!message.content.url || isImageUrlNotCompatible(message.content.url)) return null
      return {
        type: 'image',
        image: {
    ...
    case BubbleBlockType.VIDEO: {
      ...
      if (embeddableVideoTypes.includes(message.content.type as EmbeddableVideoBubbleContentType))
        return {
          type: 'text',
          text: {
            body: `${embedBaseUrls[message.content.type as EmbeddableVideoBubbleContentType]}/${
              message.content.id
            }`,
            preview_url: true,
          },
...
const isImageUrlNotCompatible = (url: string) =>
  !isHttpUrl(url) || isGifFileUrl(url) || isSvgSrc(url)

const isHttpUrl = (text: string) => text.startsWith('http://') || text.startsWith('https://')

const isGifFileUrl = (url: string) => {
