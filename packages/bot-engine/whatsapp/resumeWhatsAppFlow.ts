...
import redis from '@quickbot.io/lib/redis'
import { downloadMedia } from './downloadMedia'
import { InputBlockType } from '@quickbot.io/schemas/features/blocks/inputs/constants'
import { uploadFileToBucket } from '@quickbot.io/lib/s3/uploadFileToBucket'
import { getBlockById } from '@quickbot.io/schemas/helpers'
import { getSettingsPublicBotForWhatsappCredentials } from '../queries/getSettingsPublicBotForWhatsappCredentials'
...
export const resumeWhatsAppFlow = async ({
  ...
  const messageSentBefore3MinutesAgo = messageSendDate.getTime() < Date.now() - 180000
  ...
  const { incomingMessages, isReplyingWasSet } = await aggregateParallelMediaMessagesIfRedisEnabled(
    {
      receivedMessage,
  ...
  const botInfo = session?.state.botsQueue[0]
  const currentBot = botInfo?.bot
  const { block } =
    (currentBot && session?.state.currentBlockId
      ? getBlockById(session.state.currentBlockId, currentBot.groups)
      : undefined) ?? {}
  ...
  const publicBot =
    workspaceId && credentialsId
      ? await getSettingsPublicBotForWhatsappCredentials({ workspaceId, credentialsId })
      : undefined
  ...
  await sendChatReplyToWhatsApp({
    ...
    incomingMessageId: receivedMessage.id,
    sessionId: sessionId,
    botSettings: publicBot?.settings,
  })
...
const getIncomingMessageContent = async ({
  ...
  for (const message of messages) {
    switch (message.type) {
      ...
      case 'interactive': {
        // Handle WhatsApp Flow responses for multiple choice
        if (message.interactive.nfm_reply) {
          ...
          if (flowResponse.response_json) {
            try {
              ...
              if (responseData.selected_titles) {
                text = responseData.selected_titles
              } else if (
                responseData.selected_options &&
                Array.isArray(responseData.selected_options)
              ) {
                // Fallback: join selected option IDs or values
                text = responseData.selected_options.join(', ')
              } else {
                text = flowResponse.response_json
              }
            } catch (error) {
      ...
      case 'image': {
        ...
        if (fileVisibility !== 'Public') {
          fileUrl =
            env.NEXTAUTH_URL +
            `/api/bots/${botId}/whatsapp/media/${workspaceId ? `` : 'preview/'}${mediaId}`
        } else {
          ...
          const url = await uploadFileToBucket({
            file,
            key:
              resultId && workspaceId && botId
                ? `public/workspaces/${workspaceId}/analytics/${botId}/answers/${resultId}/${mediaId}`
...
const getCredentials = async ({
  ...
  if (isPreview) {
    if (!env.META_SYSTEM_USER_TOKEN || !env.WHATSAPP_PREVIEW_FROM_PHONE_NUMBER_ID) return
    return {
...
const aggregateParallelMediaMessagesIfRedisEnabled = async ({
  ...
  if (redis && ['document', 'video', 'image'].includes(receivedMessage.type)) {
    ...
    try {
      ...
      await new Promise((resolve) => setTimeout(resolve, incomingMessageDebounce))
      ...
      return {
        ...
        incomingMessages: newMessagesResponse.map((msgStr) => JSON.parse(msgStr)),
      }
    } catch (error) {
