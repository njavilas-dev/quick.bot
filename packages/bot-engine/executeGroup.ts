...
import {
  ...
  isIntegrationBlock,
  isLogicBlock,
} from '@quickbot.io/schemas/helpers'
...
> => {
  ...
  const messages: ContinueChatResponse['messages'] = currentReply?.messages ?? []
  let clientSideActions: ContinueChatResponse['clientSideActions'] = currentReply?.clientSideActions
  ...
  for (const block of group.blocks) {
    if (newStartTime && env.CHAT_API_TIMEOUT && Date.now() - newStartTime > env.CHAT_API_TIMEOUT) {
      throw new TRPCError({
        code: 'TIMEOUT',
    ...
    if (isBubbleBlock(block)) {
      ...
      const message = parseBubbleBlock(block as BubbleBlockWithDefinedContent, {
        ...
        botVersion: newSessionState.botsQueue[0].bot.version,
        textBubbleContentFormat,
      })
    ...
    if (
      ...
      if (!newSessionState.botsQueue[0].resultId)
        newSessionState = {
          ...newSessionState,
          previewMetadata: {
            ...newSessionState.previewMetadata,
            setVariableHistory: (newSessionState.previewMetadata?.setVariableHistory ?? []).concat(
    ...
    if (executionResponse.logs) logs = [...(logs ?? []), ...executionResponse.logs]
    if (executionResponse.newSessionState) newSessionState = executionResponse.newSessionState
    if ('clientSideActions' in executionResponse && executionResponse.clientSideActions) {
      clientSideActions = [
        ...(clientSideActions ?? []),
      ...
      if ('customEmbedBubble' in executionResponse && executionResponse.customEmbedBubble) {
        messages.push({
          id: createId(),
      ...
      if (
        executionResponse.clientSideActions?.find((action) => action.expectsDedicatedReply) ||
        ('customEmbedBubble' in executionResponse && executionResponse.customEmbedBubble)
      ) {
        return {
          messages,
          newSessionState: {
  ...
  if (!nextEdgeId && newSessionState.botsQueue.length === 1)
    return {
      messages,
      newSessionState,
      clientSideActions,
