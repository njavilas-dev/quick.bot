import { BotTemplateNotificationEmail, render } from '@quickbot.io/emails'
import {
...
import { byId, isDefined, isEmpty, isNotDefined, omit } from '@quickbot.io/lib'
...
export const executeSendEmailBlock = async (
  ...
  const logs: ChatLog[] = []
  const { options } = block
  const { bot, resultId, answers } = state.botsQueue[0]
  ...
  const bodyUniqueVariable = findUniqueVariable(bot.variables)(options?.body)?.value
  const body = bodyUniqueVariable
    ? stringifyUniqueVariableValueAsHtml(bodyUniqueVariable)
    : parseVariables(bot.variables, { isInsideHtml: !options?.isBodyCode })(options?.body ?? '')

  if (!options?.recipients) return { outgoingEdgeId: block.outgoingEdgeId, logs }

  try {
    const sendEmailLogs = await sendEmail({
      bot,
      answers,
      credentialsId: options.credentialsId ?? defaultSendEmailOptions.credentialsId,
      recipients: options.recipients.map(parseVariables(bot.variables)),
      subject: options.subject ? parseVariables(bot.variables)(options?.subject) : undefined,
      body,
      cc: options.cc ? options.cc.map(parseVariables(bot.variables)) : undefined,
      bcc: options.bcc ? options.bcc.map(parseVariables(bot.variables)) : undefined,
      replyTo: options.replyTo ? parseVariables(bot.variables)(options.replyTo) : undefined,
      fileUrls: getFileUrls(bot.variables)(options.attachmentsVariableId),
...
const sendEmail = async ({
  ...
  const email: Mail.Options = {
    ...
    attachments: fileUrls
      ? (typeof fileUrls === 'string' ? fileUrls.split(', ') : fileUrls).map((url) => ({
        path: url,
      }))
      : undefined,
    ...emailBody,
...
const stringifyUniqueVariableValueAsHtml = (value: Variable['value']): string => {
  if (!value) return ''
