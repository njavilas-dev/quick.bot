...
import { JSONParse } from '@quickbot.io/lib/JSONParse'
...
export const executeWebhookBlock = async (
  ...
  const parsedWebhook = await parseWebhookAttributes({
    ...
    answers: state.botsQueue[0].answers,
  })
  if (!parsedWebhook) {
    logs.push({
      status: 'error',
...
export const parseWebhookAttributes = async ({
  ...
  const basicAuthHeaderIdx = webhook.headers?.findIndex(
    (h) => h.key?.toLowerCase() === 'authorization' && h.value?.toLowerCase()?.includes('basic'),
  )
  ...
  const headers = convertKeyValueTableToObject(webhook.headers, bot.variables) as
    | ExecutableHttpRequest['headers']
    | undefined
  const queryParams = stringify(
    convertKeyValueTableToObject(webhook.queryParams, bot.variables, true),
    { indices: false },
  )
  ...
  return {
    url: parseVariables(bot.variables)(webhook.url + (queryParams !== '' ? `?${queryParams}` : '')),
    basicAuth,
    method,
    headers,
    body,
...
export const executeWebhook = async (
  ...
  const isLongRequest = params.disableRequestTimeout
    ? true
    : longReqTimeoutWhitelist.some((whiteListedUrl) => url?.includes(whiteListedUrl))
  ...
  try {
    ...
    if (error instanceof HTTPError) {
      const responseBody = error.response.headers.get('content-type')?.includes('json')
        ? await error.response.json()
        : await error.response.text()
      const response = {
        statusCode: error.response.status,
        data: typeof responseBody === 'string' ? safeJsonParse(responseBody).data : responseBody,
      }
      logs.push({
        status: 'error',
    ...
    if (error instanceof TimeoutError) {
      const response = {
        statusCode: 408,
        data: {
          message: `Request timed out. (${(request.timeout ? request.timeout : 0) / 1000}ms)`,
        },
      }
      logs.push({
        status: 'error',
    ...
    const response = {
      statusCode: 500,
      data: { message: `Error from QuickBot server: ${error}` },
    }
    console.error(error)
    logs.push({
      status: 'error',
