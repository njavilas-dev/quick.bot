import { AbTestBlock, SessionState } from '@quickbot.io/schemas'
import { ExecuteLogicResponse } from '../../../types'
import { defaultAbTestOptions } from '@quickbot.io/schemas/features/blocks/logic/abTest/constants'

export const executeAbTest = (_: SessionState, block: AbTestBlock): ExecuteLogicResponse => {
  const aEdgeId = block.items[0].outgoingEdgeId
  const random = Math.random() * 100
  if (random < (block.options?.aPercent ?? defaultAbTestOptions.aPercent) && aEdgeId) {
