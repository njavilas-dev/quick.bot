...
export const parseDateReply = (reply: string, block: DateInputBlock): ParsedReply => {
  const parsedDate = (block.options?.format ?? defaultDateInputOptions.format).startsWith('dd')
  ...
  const detectedStartDate = parseDateWithNeutralTimezone(parsedDate[0].start.date())
  ...
  const detectedEndDate = parsedDate[0].end?.date()
    ...
    : undefined
  const endDate = detectedEndDate ? format(detectedEndDate, formatString) : undefined
  ...
  if (
    isDefined(max) &&
    (detectedStartDate > new Date(max) || (detectedEndDate && detectedEndDate > new Date(max)))
  )
    return { status: 'fail' }
  ...
  if (
    isDefined(min) &&
    (detectedStartDate < new Date(min) || (detectedEndDate && detectedEndDate < new Date(min)))
  )
    return { status: 'fail' }
