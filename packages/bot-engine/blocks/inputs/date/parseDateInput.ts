...
import { DateInputBlock, SessionState, Variable } from '@quickbot.io/schemas'
...
export const parseDateInput = (state: SessionState) => (block: DateInputBlock) => {
  ...
  return {
    ...
    options: {
      ...
      min: parseDateLimit(block.options.min, block.options.hasTime, variables),
      max: parseDateLimit(block.options.max, block.options.hasTime, variables),
    },
...
const parseDateLimit = (
  ...
  if (matchDateTime) return hasTime ? matchDateTime[0] : matchDateTime[0].slice(0, 10)
  return parsedLimit
}
