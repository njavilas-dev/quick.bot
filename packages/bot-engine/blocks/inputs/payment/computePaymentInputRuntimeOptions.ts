...
const createStripePaymentIntent =
  (state: SessionState) =>
  async (options: PaymentInputBlock['options']): Promise<PaymentInputRuntimeOptions> => {
    const {
      resultId,
      bot: { variables },
    } = state.botsQueue[0]
    ...
    if (isNaN(amount))
      throw new TRPCError({
        code: 'BAD_REQUEST',
        message: 'Could not parse amount, make sure your block is configured correctly',
      })
    // Create a PaymentIntent with the order amount and currency
    const receiptEmail = parseVariables(variables)(options.additionalInformation?.email)
    const paymentIntent = await stripe.paymentIntents.create({
      amount,
      currency,
      receipt_email: receiptEmail === '' ? undefined : receiptEmail,
      description: parseVariables(variables)(options.additionalInformation?.description),
    ...
    const priceFormatter = new Intl.NumberFormat(options.currency === 'EUR' ? 'fr-FR' : undefined, {
      style: 'currency',
      currency,
    })
    ...
    return {
      ...
      amountLabel: priceFormatter.format(amount / (isZeroDecimalCurrency(currency) ? 1 : 100)),
    }
  }
...
const getStripeInfo = async (
  ...
  return (await decrypt(credentials.data, credentials.iv)) as StripeCredentials['data']
}
