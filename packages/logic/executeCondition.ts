import { isNotDefined, isDefined } from '@quickbot.io/lib'
import { Comparison, Condition, Variable } from '@quickbot.io/schemas'
...
export const executeCondition = ({ condition, variables }: Props): boolean => {
  ...
  return (condition.logicalOperator ?? defaultConditionItemContent.logicalOperator) ===
    LogicalOperator.AND
...
const executeComparison =
  ...
  (comparison: Comparison): boolean => {
    ...
    const inputValue = variables.find((v) => v.id === comparison.variableId)?.value ?? null
    const value =
    ...
    switch (comparison.comparisonOperator) {
      case ComparisonOperators.CONTAINS: {
        ...
        const contains = (a: string | null, b: string | null) => {
          ...
          return a.toLowerCase().trim().normalize().includes(b.toLowerCase().trim().normalize())
        }
      ...
      case ComparisonOperators.STARTS_WITH: {
        const startsWith = (a: string | null, b: string | null) => {
          if (b === '' || !b || !a) return false
          return a.toLowerCase().trim().normalize().startsWith(b.toLowerCase().trim().normalize())
        }
      ...
      case ComparisonOperators.ENDS_WITH: {
        const endsWith = (a: string | null, b: string | null) => {
          if (b === '' || !b || !a) return false
          return a.toLowerCase().trim().normalize().endsWith(b.toLowerCase().trim().normalize())
        }
...
const compare = (
  ...
  if (type === 'every') return a.every((a) => b.every((b) => compareStrings(a, b)))
...
const preprocessRegex = (regex: string) => {
  ...
  if (regexWithFlags) return { pattern: regexWithFlags[1], flags: regexWithFlags[2] }
