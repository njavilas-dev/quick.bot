import { forgedBlockIds } from '@quickbot.io/forge-repository/constants'
import {
  Block,
  InputBlock,
  BubbleBlock,
...
import { Group } from './features/bot/types'
...
const isMediaBubbleBlock = (block: Block): block is ImageBubbleBlock | VideoBubbleBlock =>
  block.type === BubbleBlockType.IMAGE || block.type === BubbleBlockType.VIDEO

const isTextInputBlock = (block: Block): block is TextInputBlock =>
...
const isSingleChoiceInput = (block: Block): block is ChoiceInputBlock =>
  block.type === InputBlockType.CHOICE &&
  'options' in block &&
...
export const isIntegrationBlock = (block: Block): block is IntegrationBlock =>
  (Object.values(IntegrationBlockType).concat(forgedBlockIds as any[]) as any[]).includes(
    block.type,
  )
...
const isBubbleBlockType = (type: Block['type']): type is BubbleBlockType =>
  (Object.values(BubbleBlockType) as string[]).includes(type)

export const blockHasOptions = (block: Block): block is BlockWithOptions => 'options' in block

export const blockTypeHasItems = (
  type: Block['type'],
): type is LogicBlockType.CONDITION | InputBlockType.CHOICE | LogicBlockType.AB_TEST =>
  type === LogicBlockType.CONDITION ||
  type === InputBlockType.CHOICE ||
  type === LogicBlockType.AB_TEST

export const blockHasItems = (block: Block): block is ConditionBlock | ChoiceInputBlock =>
