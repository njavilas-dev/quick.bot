...
import { settingsSchema, themeSchema } from '../bot'
import {
...
import { botV5Schema, botV6Schema } from '../bot'
import { BubbleBlockType } from '../blocks/bubbles/constants'
import { clientSideActionSchema } from './clientSideAction'
import { ChatSession as ChatSessionFromPrisma } from '@quickbot.io/prisma'
...
const chatSessionSchema = z.object({
  ...
  isReplying: z
    .boolean()
    .nullable()
    .describe('Used in WhatsApp runtime to avoid concurrent replies from the bot'),
  lastWhatsAppMessageId: z
    .string()
    .nullable()
    .describe('ID of the last message sent by WhatsApp, used to avoid duplicate responses'),
...
export const startBotSchema = z.preprocess(
  preprocessBot,
  z.discriminatedUnion('version', [
    botV5Schema._def.schema.pick(startBotPick).openapi({
      title: 'Bot V5',
      ref: 'botV5',
    }),
    botV6Schema.pick(startBotPick).openapi({
      title: 'Bot V6',
      ref: 'botV6',
    }),
  ]),
)
export type StartBot = z.infer<typeof startBotSchema>
...
export const startChatInputSchema = z.object({
  publicId: z
    .string()
    .describe(
      "[Where to find my bot's public ID?](https://docs.quick.bot/api/authentication#how-to-find-my-publicid)",
    ),
  ...
  prefilledVariables: z
    ...
    .optional()
    .describe(
      '[More info about prefilled variables.]https://docs.quick.bot/builder/editor/variables#prefilled-variables../editor/variables#prefilled-variables)',
    )
    .openapi({
      example: {
        'First name': 'John',
...
export const startPreviewChatInputSchema = z.object({
  ...
  bot: startBotSchema
    .optional()
    .describe('If set, it will override the bot that is used to start the chat.'),
...
export const botInChatReply = z.preprocess(
  preprocessBot,
  z.discriminatedUnion('version', [
    botV5Schema._def.schema.pick(botInChatReplyPick),
    botV6Schema.pick(botInChatReplyPick),
  ]),
)
...
export const startChatResponseSchema = z
  .object({
    sessionId: z.string().describe('To save and use for /continueChat requests.'),
