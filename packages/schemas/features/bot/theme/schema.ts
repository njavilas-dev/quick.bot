import { BotThemeTemplate as ThemeTemplatePrisma } from '@quickbot.io/prisma'
import { z } from '../../../zod'
import {
  BackgroundType,
...
const containerBorderThemeSchema = z.object({
  ...
  roundeness: z.enum(borderRoundnessValues).optional(),
...
const progressBarSchema = z.object({
  ...
  color: z.string().optional(),
  backgroundColor: z.string().optional(),
  placement: z.enum(progressBarPlacementValues).optional(),
  thickness: z.number().optional(),
  position: z.enum(progressBarPositionValues).optional(),
})
