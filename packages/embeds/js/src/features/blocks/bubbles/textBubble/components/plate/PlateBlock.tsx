    ...
    <Match when={true}>
      <Switch>
        <Match when={props.element.type === 'a'}>
          <a href={props.element.url as string} target="_blank" rel="noopener noreferrer">
            <For each={props.element.children as TDescendant[]}>
              {(child) => (
                <PlateElement
                  element={child}
                  isUniqueChild={(props.element.children as TDescendant[])?.length === 1}
                />
              )}
            </For>
          </a>
            ...
            <For each={props.element.children as TDescendant[]}>
              {(child) => (
                <PlateElement
                  element={child}
                  isUniqueChild={(props.element.children as TDescendant[])?.length === 1}
                  insideInlineVariable={props.element.type === 'inline-variable'}
                />
              )}
            </For>
          </ElementRoot>
...
const ElementRoot = (props: ElementRootProps) => {
  return (
    <Switch>
      <Match when={props.element.type === 'inline-variable' || props.insideInlineVariable}>
