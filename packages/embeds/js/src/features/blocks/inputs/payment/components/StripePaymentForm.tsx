...
export const StripePaymentForm = (props: Props) => {
  ...
  const handleSubmit = async (event: Event & { submitter: HTMLElement }) => {
    ...
    setPaymentInProgressInStorage({
      ...
      bot: props.context.bot,
    })
    const { postalCode, ...address } = props.options?.additionalInformation?.address ?? {}
  ...
  return (
    <form
      ...
      class="flex flex-col p-4 input w-full items-center"
    >
      ...
      <Show when={isMounted()}>
        ...
        >
          {props.options?.labels?.button ?? defaultPaymentInputOptions.labels.button}{' '}
          {props.options?.amountLabel}
        </SubmitButton>
      </Show>

      <Show when={message()}>
        <div class="mt-4 text-center animate-fade-in input__error-message">{message()}</div>
      </Show>
    </form>
  )
}
