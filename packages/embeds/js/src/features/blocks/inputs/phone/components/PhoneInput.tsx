...
import { phoneCountries } from '@quickbot.io/lib/phoneCountries'
...
export const PhoneInput = (props: PhoneInputProps) => {
  ...
  const handleInput = (inputValue: string | undefined) => {
    ...
    if ((inputValue === '' || inputValue === '+') && selectedCountryCode() !== 'INT')
      setSelectedCountryCode('INT')
    const matchedCountry =
      inputValue?.startsWith('+') &&
      inputValue.length > 2 &&
      phoneCountries.reduce<(typeof phoneCountries)[number] | null>((matchedCountry, country) => {
  ...
  const selectNewCountryCode = (event: Event & { currentTarget: { value: string } }) => {
  ...
  return (
    <InputFormLayout onSubmit={submit} buttonLabel={props.labels?.button}>
      <div class="relative flex justify-center items-center input__country-select">
      ...
      <ShortTextInput
        ...
        value={inputValue()}
        onInput={handleInput}
        placeholder={props.labels?.placeholder ?? defaultPhoneInputOptions.labels.placeholder}
        autofocus={!isMobile()}
      />
    </InputFormLayout>
  )
