...
import type { RatingInputBlock } from '@quickbot.io/schemas'
import { createSignal, For, Match, Switch, Show } from 'solid-js'
import { isDefined, isEmpty, isNotDefined } from '@quickbot.io/lib'
  ...
  return (
    <form class="flex flex-col gap-2" onSubmit={handleSubmit}>
      {props.block.options?.labels?.left && (
        <span class="text-sm w-full">{props.block.options.labels.left}</span>
      )}
      <div class="flex flex-wrap justify-center gap-2">
        <For
          each={Array.from(
            Array(
              (props.block.options?.length ?? defaultRatingInputOptions.length) +
              ((props.block.options?.buttonType ?? defaultRatingInputOptions.buttonType) ===
                'Numbers'
                ? -(
      ...
      {props.block.options?.labels?.right && (
        <span class="text-sm w-full text-right pr-2">{props.block.options.labels.right}</span>
      )}

      <div class="flex justify-end">
        {isDefined(rating()) && (
          <SubmitButton>
            {props.block.options?.labels?.button ?? defaultRatingInputOptions.labels.button}
          </SubmitButton>
        )}
      </div>
    </form>
...
const RatingButton = (props: RatingButtonProps) => {
  ...
  return (
    <Switch>
      <Match when={(props.buttonType ?? defaultRatingInputOptions.buttonType) === 'Numbers'}>
        ...
        <Show when={!props.isOneClickSubmitEnabled}>
          <div
            ...
            class={
              'py-2 px-4 font-normal focus:outline-none cursor-pointer select-none button--selectable' +
              (isDefined(props.rating) && props.idx <= props.rating ? ' button--selected' : '')
            }
          >
            {props.idx}
          </div>
      ...
      <Match when={(props.buttonType ?? defaultRatingInputOptions.buttonType) !== 'Numbers'}>
        <div
          class={
            'flex justify-center items-center cursor-pointer w-[42px] h-[42px] transition-all duration-100 hover:brightness-90 active:brightness-75 input__rating-icon-container' +
            (isDefined(props.rating) && props.idx <= props.rating
              ? 'input__rating-icon-container--selected'
              : '')
          }
          innerHTML={
