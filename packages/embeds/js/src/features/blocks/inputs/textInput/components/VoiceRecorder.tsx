...
import { hexToRgb } from '@quickbot.io/lib/hexToRgb'
...
export const VoiceRecorder = (props: Props) => {
  ...
  let bars: number[] = []
  let recordTimeInterval: ReturnType<typeof setInterval> | undefined
  ...
  const startRecording = async () => {
    ...
    volumeNode.port.onmessage = (event) => {
      ...
      const shouldAddNewBar =
        (initBars + bars.length) * (barWidth + barGap) < canvasElement.width + offset
      if (shouldAddNewBar)
        bars.push(
          Math.min(Math.max(event.data, minDetectedVolumePercent), maxDetectedVolumePercent),
        )
    }
  ...
  return (
    <div
      class={clsx(
        'w-full gap-2 items-center transition-opacity px-2 recorder',
        props.recordingStatus === 'started' ? 'opacity-1 flex' : 'opacity-0 hidden',
      )}
    >
      <button class="p-0.5 rounded-full" on:click={stopRecording} aria-label="Stop recording">
        <CloseIcon class="w-4" />
      </button>
      <div class="relative flex w-full">
        <canvas ref={canvasElement} class="w-full h-[56px]" />
        <div class="absolute w-2 left-0 h-[56px] recorder__left-gradient" />
        <div class="absolute w-3 right-0 h-[56px] recorder__right-gradient" />
      </div>
      <span class="flex-none w-[35px] font-bold text-sm">{formatTimeLabel(recordingTime())}</span>
    </div>
  )
}
