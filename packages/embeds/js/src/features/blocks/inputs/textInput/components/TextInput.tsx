...
import { getRuntimeVariable } from '@quickbot.io/env/getRuntimeVariable'
...
export const TextInputTest = (props: Props) => {
  ...
  const [recordingStatus, setRecordingStatus] = createSignal<'started' | 'asking' | 'stopped'>(
    'stopped',
  )
    ...
    if (checkIfInputIsValid()) {
      ...
      if (selectedFiles().length > 0) {
        ...
        const urls = await uploadFiles({
          apiHost: props.context.apiHost ?? guessApiHost({ ignoreChatApiUrl: true }),
  ...
  const handleRecordingConfirmed = (stream: MediaStream) => {
    ...
    mediaRecorder.onstop = async () => {
      ...
      const blob = await fixWebmDuration(new Blob(recordedChunks, { type: mimeType }), duration)

      const audioFile = new File(
        [blob],
        `rec-${props.block.id}-${Date.now()}.${mimeType === 'audio/webm' ? 'webm' : 'mp4'}`,
        {
          type: mimeType,
        },
      )
  ...
  const isDisabled = Boolean(uploadProgress())
  ...
  const customButton = (
    <Switch>
      ...
      >
        <Button on:click={recordVoice} aria-label="Record voice">
          <MicrophoneIcon />
        </Button>
      </Match>
      <Match when={true}>
  ...
  return (
    ...
    >
      <div
        class={clsx('input flex-col w-full relative', isDraggingOver() && 'filter brightness-95')}
      ...
      >
        <VoiceRecorder
          ...
          buttonsTheme={props.context.bot.theme.chat?.buttons}
        ...
        <Show when={recordingStatus() !== 'started'}>
          ...
          <div class="flex justify-between h-full max-h-[150px] items-center" ref={containerRef}>
            <Textarea
              containerRef={containerRef as HTMLDivElement}
            ...
            >
              <TextInputAddFileButton onNewFiles={onNewFiles} class="ml-2" />
            </Show>
          </div>
        </Show>
      </div>
