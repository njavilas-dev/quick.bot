...
import { createSignal, onMount, Show, splitProps, createEffect } from 'solid-js'
...
export const Popup = (props: PopupProps) => {
  ...
  onMount(() => {
    if (popupProps.defaultOpen || getPaymentInProgressInStorage() || getBotOpenedStateFromStorage())
      openBot()
  ...
  const toggleBot = () => {
    return isBotOpened() ? closeBot() : openBot()
  }
  ...
  return (
    <Show when={isBotOpened()}>
      <EnvironmentProvider value={document.querySelector('quickbot-popup')?.shadowRoot as Node}>
        <style>{styles}</style>
        <div
          class="relative"
