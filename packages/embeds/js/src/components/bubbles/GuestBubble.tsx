...
import { InputSubmitContent, RecordingInputSubmitContent, TextInputSubmitContent } from '@/types'
...
export const GuestBubble = (props: Props) => {
  return (
    <div
      class="flex justify-end items-end animate-fade-in gap-2"
      style={{
        'margin-left': props.hasHostAvatar ? (isMobile() ? '28px' : '50px') : undefined,
      }}
    >
      <Switch>
        ...
        <Match when={props.answer?.type === 'recording'}>
          <AudioGuestBubble answer={props.answer as RecordingInputSubmitContent} />
        </Match>
      </Switch>
...
const TextGuestBubble = (props: { answer: TextInputSubmitContent }) => {
  ...
  return (
    <div class="flex flex-col gap-1 items-end w-full">
          ...
          >
            {(attachment, idx) => (
              <img
                ...
                class={clsx(
                  'cursor-pointer bubble--guest__image-attachment',
      ...
      <div
        class="p-[1px] whitespace-pre-wrap max-w-full flex flex-col bubble--guest"
        data-testid="guest-bubble"
      >
        <Show when={isNotEmpty(props.answer.label ?? props.answer.value)}>
          <span class="px-[15px] py-[7px]" style={{ 'overflow-wrap': 'break-word' }}>
            {props.answer.label ?? props.answer.value}
          </span>
        </Show>
      </div>
      <Modal isOpen={clickedImageSrc() !== undefined} onClose={() => setClickedImageSrc(undefined)}>
        <img src={clickedImageSrc()} alt="Attachment" style={{ 'border-radius': '6px' }} />
      </Modal>
    </div>
  )
}
...
const AudioGuestBubble = (props: { answer: RecordingInputSubmitContent }) => {
  return (
    <div class="flex flex-col gap-1 items-end">
      <div
        class="p-2 w-full whitespace-pre-wrap flex flex-col bubble--guest"
        data-testid="guest-bubble"
      >
