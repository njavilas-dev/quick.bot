...
const updateUserEmail = async () => {
  ...
  if (!currentUserEmail || !newEmail || isCancel(currentUserEmail) || isCancel(newEmail))
    throw new Error('Invalid emails')

  const existingUserWithNewEmail = await prismaWithLogging.user.findUnique({
    where: {
      email: newEmail,
    },
  })
  ...
  const user = await prismaWithLogging.user.update({
    where: {
