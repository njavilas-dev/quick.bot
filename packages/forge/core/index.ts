...
export const createAuth = <A extends AuthDefinition>(authDefinition: A) => authDefinition
...
export const createVersionedBlock = <
  Blocks extends Record<string, BlockDefinition<string, AuthDefinition, z.ZodObject<any>>>,
>(
  blocks: Blocks,
): Blocks => blocks
...
export const option = {
  ...
  enum: <T extends string>(values: readonly [T, ...T[]]) => Object.assign(z.enum(values).optional(), {
    default: (value: T) => Object.assign(z.enum(values).default(value).optional(), {
      openapi: (params: any) => z.enum(values).default(value).openapi(params).optional(),
    }),
  }),
  number: Object.assign(z.number().optional(), {
    min: (value: number) => Object.assign(z.number().min(value).optional(), {
      max: (maxValue: number) => z.number().min(value).max(maxValue).optional(),
      layout: (metadata: ZodLayoutMetadata<ZodTypeAny>) => z.number().min(value).optional().layout(metadata),
    }),
    max: (value: number) => Object.assign(z.number().max(value).optional(), {
      min: (minValue: number) => z.number().min(minValue).max(value).optional(),
      layout: (metadata: ZodLayoutMetadata<ZodTypeAny>) => z.number().max(value).optional().layout(metadata),
    }),
  }),
  array: <T extends z.ZodTypeAny>(schema: T) => z.array(schema).optional(),
  discriminatedUnion: <
    T extends string,
    J extends [z.ZodDiscriminatedUnionOption<T>, ...z.ZodDiscriminatedUnionOption<T>[]],
  >(
    field: T,
    schemas: J,
  ) =>
    // @ts-expect-error
    z.discriminatedUnion<T, J>(field, [z.object({ [field]: z.undefined() }), ...schemas]),
      ...
      .object({
        comparisons: z.array(
          z.object({
            ...
            operator: z.enum(operators).optional().layout({ defaultValue: 'Equal to' }),
            value: z.string().optional().layout({ placeholder: 'Enter a value' }),
          }),
        ),
