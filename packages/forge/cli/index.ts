...
const main = async () => {
  p.intro('Create a new QuickBot integration block')
  const { name, id } = await p.group(
    {
      ...
      id: ({ results }) =>
        p.text({
          message: 'Integration ID? (should be a slug like: cal-com, openai...)',
          placeholder: 'my-integration',
          defaultValue: slugify(results.name ?? ''),
          validate: (val) => {
            if (!slugRegex.test(val)) return 'Invalid ID. Must be a slug, like: "google-sheets".'
          },
        }),
    },
    {
  ...
  s.stop('Creating files...')
  // Skip installation step since this causes issues and is unnecessary
  // The block has been created, and the project's main package manager will handle the workspace dependencies
  s.stop('Creating files...')
  p.outro(`Done! 🎉 

Block created successfully at packages/forge/blocks/${prompt.camelCaseId}

You can now:
1. Run 'bun install' from the project root to update dependencies
2. Add your actions in the new block directory
3. Start coding!`)
}
...
const createIndexFile = async (path: string, { id, camelCaseId, name, auth }: PromptResult) => {
...
const createTsConfig = async (path: string) => {
  writeFileSync(
    ...
    await prettier.format(
      JSON.stringify({
        extends: '@quickbot.io/tsconfig/base.json',
        include: ['**/*.ts', '**/*.tsx'],
...
const createLogoFile = async (path: string, { camelCaseId }: { camelCaseId: string }) => {
  writeFileSync(
    join(path, 'logo.tsx'),
    await prettier.format(
      `/** @jsxImportSource react */
...
const addBlockToRepository = async ({
  ...
  if (auth !== 'none') await addBlockToCredentialsRepoSchemas(schemasPath, camelCaseId, id)
}

const createSchemasFile = async (
  path: string,
  { id, auth }: { id: string; name: string; auth: 'apiKey' | 'encryptedData' | 'none' },
) => {
  const camelCaseName = camelize(id as string)
  writeFileSync(
...
import { ${auth !== 'none' ? 'parseBlockCredentials,' : ''
      } parseBlockSchema } from '@quickbot.io/forge'
import { ${camelCaseName}Block } from '.'${auth !== 'none' ? `\nimport { auth } from './auth'` : ''}
...
async function addBlockToRepoDefinitions(schemasPath: string, camelCaseId: string, id: string) {
  const existingDefinitionsData = readFileSync(join(schemasPath, 'definitions.ts')).toString()
  const importStatement = `import { ${camelCaseId}Block } from '@quickbot.io/${id}-block'`
  ...
  const newDefinitionsData = `${existingDefinitionsData.slice(0, nextLineImportIndex)}
${importStatement}
...
async function addBlockToRepoSchemas(schemasPath: string, camelCaseId: string, id: string) {
  const existingDefinitionsData = readFileSync(join(schemasPath, 'schemas.ts')).toString()
  const importStatement = `import { ${camelCaseId}Block } from '@quickbot.io/${id}-block'
import { ${camelCaseId}BlockSchema } from '@quickbot.io/${id}-block/schemas'`
...
async function addBlockToCredentialsRepoSchemas(
  ...
  const existingSchemasData = readFileSync(join(schemasPath, 'credentials.ts')).toString()
  const importStatement = `import { ${camelCaseId}Block } from '@quickbot.io/${id}-block'
import { ${camelCaseId}CredentialsSchema } from '@quickbot.io/${id}-block/schemas'`
  ...
  const newDefinitionsData = `${existingSchemasData.slice(0, nextLineImportIndex)}
${importStatement}
...
async function addBlockToRepoConstants(schemasPath: string, id: string) {
  const existingDefinitionsData = readFileSync(join(schemasPath, 'constants.ts')).toString()
...
async function addBlockToRepoPackageJson(id: string) {
  ...
  packageJson.devDependencies[`@quickbot.io/${id}-block`] = 'workspace:*'
  writeFileSync(
