  ...
  run: {
    stream: {
      getStreamVariableId: ({ responseMapping }) =>
        responseMapping?.find((r) => !r.item || r.item === 'Answer')?.variableId,
      run: async ({
        credentials: { apiEndpoint, apiKey },
        options: { conversation_id, conversationVariableId, query, user, inputs, responseMapping },
        variables,
      }) => {
        const existingDifyConversationId = conversationVariableId
          ...
          : conversation_id
        try {
          const response = await ky((apiEndpoint ?? defaultBaseUrl) + '/v1/chat-messages', {
            method: 'POST',
            headers: {
              ...
              async start(controller) {
                try {
                  await processDifyStream(reader, {
                    ...
                    onMessage: (message) => {
                      controller.enqueue(
                        new TextEncoder().encode(formatStreamPart('text', message)),
                      )
                    },
                    async onMessageEnd({ totalTokens, conversationId }) {
                      if (
                        ...
                        await variables.set(conversationVariableId, conversationId)
    ...
    server: async ({
      credentials: { apiEndpoint, apiKey },
      options: { conversationVariableId, conversation_id, query, user, inputs, responseMapping },
      variables,
      logs,
    }) => {
      const existingDifyConversationId = conversationVariableId
      ...
      try {
        ...
        responseMapping?.forEach((mapping) => {
          ...
          if (item === 'Answer') variables.set(mapping.variableId, convertNonMarkdownLinks(answer))
          ...
          if (item === 'Total Tokens') variables.set(mapping.variableId, totalTokens)
        })
      } catch (err) {
        if (err instanceof HTTPError) {
          return logs.add({
...
const processDifyStream = async (
  ...
  while (true) {
    ...
    for (const line of lines.filter((line) => line.length > 0 && line !== '\n')) {
      jsonChunk += line
