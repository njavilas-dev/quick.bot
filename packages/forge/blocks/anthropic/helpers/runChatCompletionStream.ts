...
import { ChatCompletionOptions } from '@quickbot.io/openai-block/shared/parseChatCompletionOptions'
...
import { pumpStreamUntilDone } from '@quickbot.io/ai/pumpStreamUntilDone'
import { appendToolResultsToMessages } from '@quickbot.io/ai/appendToolResultsToMessages'
...
export const runChatCompletionStream = async ({
  ...
  if (!modelName) return { httpError: { status: 400, message: 'model not found' } }
    ...
    return {
      stream: new ReadableStream({
        async start(controller) {
          const reader = response.toDataStream().getReader()

          await pumpStreamUntilDone(controller, reader)

          toolCalls = await response.toolCalls
          if (toolCalls.length > 0) toolResults = (await response.toolResults) as ToolResultPart[]

          while (toolCalls && toolCalls.length > 0 && totalToolCalls < maxToolRoundtrips) {
            totalToolCalls += 1
            ...
            const reader = newResponse.toDataStream().getReader()
            await pumpStreamUntilDone(controller, reader)
