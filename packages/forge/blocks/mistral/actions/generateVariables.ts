...
import { parseGenerateVariablesOptions } from '@quickbot.io/openai-block/shared/parseGenerateVariablesOptions'
import { runGenerateVariables } from '@quickbot.io/openai-block/shared/runGenerateVariables'
...
export const generateVariables = createAction({
  ...
  run: {
    server: ({ credentials, options, variables, logs }) => {
      if (credentials?.apiKey === undefined) return logs.add('No API key provided')
