...
import { defaultBaseUrl, defaultLimitForSearch, filterOperators } from '../constants'
...
export const searchRecords = createAction({
  ...
  options: option.object({
    tableId: option.string.layout({
      label: 'Table ID',
      moreInfoTooltip: 'Can be found by clicking on the 3 dots next to the table name.',
      isRequired: true,
    }),
    viewId: option.string.layout({
      label: 'View ID',
      moreInfoTooltip: 'Can be found by clicking on the 3 dots next to the view name.',
    }),
    ...
    filter: option
      .filter({
        ...
        isJoinerHidden: ({ filter }) => !filter?.comparisons || filter.comparisons.length < 2,
      })
      .layout({
        accordion: 'Filter',
      }),
  ...
  run: {
    server: async ({
      ...
      try {
        ...
        const filteredList =
          isDefined(filterIndex) && data.list[filterIndex] ? [data.list[filterIndex]] : data.list
        ...
        responseMapping?.forEach((mapping) => {
          ...
          const items = filteredList.map((item) => item[mapping.fieldName as string])

          variables.set(mapping.variableId, items.length === 1 ? items[0] : items)
        })
      } catch (error) {
