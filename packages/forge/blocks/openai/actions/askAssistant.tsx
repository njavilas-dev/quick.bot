import {
  ...
  createAction,
  option,
} from '@quickbot.io/forge'
import { isDefined, isEmpty, isNotEmpty } from '@quickbot.io/lib'
  ...
  fetchers: [
    ...
    },
    {
      ...
      fetch: async ({ options, credentials }) => {
        ...
        const response = await openai.beta.assistants.retrieve(options.assistantId)

        return response.tools
          .filter((tool) => tool.type === 'function')
          .map((tool) =>
            tool.type === 'function' && tool.function.name ? tool.function.name : undefined,
          )
          .filter(isDefined)
      },
    ...
    server: async ({
      ...
      responseMapping?.forEach((mapping) => {
        ...
        if (!mapping.item || mapping.item === 'Message') {
          variables.set(mapping.variableId, writingMessage.replace(/【.+】/g, ''))
        }
      })
    },
  },
...
const createAssistantStream = async ({
  ...
  if (threadVariableId && isNotEmpty(variables.get(threadVariableId)?.toString())) {
    ...
    const threadIdResponseMapping = responseMapping?.find((mapping) => mapping.item === 'Thread ID')
    if (threadIdResponseMapping?.variableId)
      await variables.set(threadIdResponseMapping.variableId, currentThreadId)
    else if (threadVariableId) await variables.set(threadVariableId, currentThreadId)
  }
    ...
    async ({ forwardStream }) => {
      ...
      while (
        ...
        const tool_outputs = (
          await Promise.all(
            runResult.required_action.submit_tool_outputs.tool_calls.map(async (toolCall) => {
              ...
              const functionToExecute = functions?.find((f) => f.name === toolCall.function.name)
        ...
        runResult = await forwardStream(
          openai.beta.threads.runs.submitToolOutputsStream(currentThreadId, runResult.id, {
            tool_outputs,
          }),
        )
      }
