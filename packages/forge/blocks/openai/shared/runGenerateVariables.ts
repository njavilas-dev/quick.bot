...
import { GenerateVariablesOptions, toolParametersSchema } from './parseGenerateVariablesOptions'
import { generateObject, LanguageModel } from 'ai'
import { Variable } from '@quickbot.io/variables/types'
import { z } from '@quickbot.io/forge/zod'
import { isNotEmpty } from '@quickbot.io/lib/utils'
...
export const runGenerateVariables = async ({
  ...
  // @ts-ignore: TODO: Temporarily ignoring this line to bypass TypeScript type checking
...
const convertVariablesToExtractToSchema = ({
  ...
  variablesToExtract.forEach((variableToExtract) => {
    ...
    const matchingVariable = variables.find((v) => v.id === variableToExtract.variableId)
    ...
    switch (variableToExtract.type) {
      ...
      case 'boolean':
        ...
        break
      case 'enum': {
        if (!variableToExtract.values || variableToExtract.values.length === 0) return
